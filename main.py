...
# 處理 BYTE 指令的函數，將字元或十六進位數據轉換成對應的二進位表示
...
# 處理 RESB 指令的函數，計算需要保留的位元組數
def RESB(line: str) -> int:
    return int(line)

# 處理 RESW 指令的函數，計算需要保留的字數
